...
class User(BaseModel, db.Model):
    ...
    # 生成一个属性，并带有setter和getter方法，不要跟getter方法，不要User的属性重名
    @property
    def password(self):
        return ''

    @password.setter
    def password(self, value):  #value:接受外加调用setter方法时候传入的参数数据
        self.password_hash = generate_password_hash(value)

    def check_pawword(self, password):
        """check_password_hash:检查"""
